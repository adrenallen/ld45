...
func handleSkySpawn():
	if $SkyObstacles/Timer.time_left <= 0:
		...
		junk.position = $SkyObstacles/StartPositions.get_children()[round(randf_range(0, $SkyObstacles/StartPositions.get_children().size())-1)].position
		var end = $SkyObstacles/EndPositions.get_children()[round(randf_range(0, $SkyObstacles/EndPositions.get_children().size())-1)]
		...
		junk.find_child("Area2D").connect("body_entered", Callable(self, "hitObstacle"))
		$SkyObstacles/Obstacles.add_child(junk)
		$SkyObstacles/Timer.start(randf_range(2,7))
...
func handleGroundSpawn():
	...
	if sceneToUse != null:
		if $GroundObstacles/Timer.time_left <= 0:
			# Yikes, don't judge me
			var mtn = sceneToUse.instantiate()
			mtn.position = $GroundObstacles/StartPosition.position
			mtn.position.x += randf_range(-50,100)
			mtn.position.y += randf_range(-32,32)
			mtn.scale = Vector2(randf_range(0.8,3),randf_range(1,6))
			mtn.find_child("Area2D").connect("body_entered", Callable(self, "hitObstacle"))
			$GroundObstacles/Obstacles.add_child(mtn)
			$GroundObstacles/Timer.start(randf_range(0.2,3))
